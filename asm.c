...
static void asm_warning(int line_no, const char *msg);

static int is_internal_label(const char *name)
{
    const char *p;
    if (name[0] != '_' || name[1] != 'L' || !name[2])
        return 0;
    for (p = name + 2; *p; p++)
        if (*p < '0' || *p > '9')
            return 0;
...
static int resolve_label_address(const char *token, int pass, int line_no)
{
    ...
    if (address < 0) {
        if (pass == 2) {
            /*
             * _L* labels are owned by cc.c and must always be defined.
             * Other names may be external symbols supplied by a later
             * kernel-link step; keep the assembler usable for translation
             * units while making the unresolved relocation visible.
             */
            if (is_internal_label(token)) {
                asm_error(line_no, "Tanımsız iç label");
                return -1;
            }
            asm_warning(line_no, "Tanımsız dış sembol (0 olarak bağlandı)");
        }
        return 0;
    }
    return address;
...
static void asm_warning(int line_no, const char *msg)
{
    fprintf(stderr, "[UYARI] Satır %d: %s\n", line_no, msg);
}
...
static int check_imm11(int32_t v, int line_no) { return 1; }
...
#define REQ_IMM_OR_LABEL(imm_var, is_branch)                        \
    do {                                                             \
        ...
        if (parse_number(_t, &_v)) {                                 \
            ...
            int32_t _addr = resolve_label_address(_t, pass, line_no); \
            if (_addr < 0) return 0;                                 \
            if (is_branch)                                           \
...

    {
        ...
        if (strcmp(tok, ".WORD") == 0) {
            ...
            if (!parse_number64(value_token, &value)) {
                int32_t address = resolve_label_address(value_token, pass,
                                                        line_no);
        ...
        /* ══ GÜVENLİK UZANTISI KOMUTLARI (Oxalyn-64 SEC) ══════ */
            ...
            insn = encode(OP_WFI, 0, 0, 0, 0);
                   ...
                   strcmp(tok, "FENCE.I") == 0 ||
                   strcmp(tok, "FENCE.RW") == 0) {
