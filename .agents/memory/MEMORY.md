- [Compiler validation scope](compiler-validation.md) — Kernel unit scans include existing variadic/function-pointer/type-parser limits beyond the supported array and initializer features.
- [Compiler lvalue addresses](compiler-lvalue-addresses.md) — keep array and struct lvalue addresses separate from loaded values through assignment evaluation.
...
- [RTL regression scope](rtl-regression-scope.md) — interrupt pipeline work uses dedicated timer/UART benches; the legacy smoke bench fails unchanged on HEAD
