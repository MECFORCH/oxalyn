...
#define TEMP_TOP     24    /* R24 = son geçici register */
#define REG_RA       31
#define REG_SP       30
#define REG_FP       29
...
static const char *string_add(const char *text)
{
    int i;
    for (i = 0; i < nstrings; i++)
        if (!strcmp(strings[i].text, text))
    ...
    snprintf(strings[nstrings].label, sizeof(strings[nstrings].label),
             "_STR_%d", nstrings);
    strncpy(strings[nstrings].text, text, sizeof(strings[nstrings].text) - 1);
    strings[nstrings].text[sizeof(strings[nstrings].text) - 1] = '\0';
...
static void emit_label(int n) { fprintf(out, "_L%d:\n", n); }
...
static int64_t ce_primary(void)
{
    ...
    if (!strncmp(ce_ptr, "UINT32_C", 8) ||
        ...
        const char *open = strchr(ce_ptr, '(');
        if (open) ce_ptr = open + 1;
        value = ce_primary();
        ce_ws();
        if (*ce_ptr == ')') ce_ptr++;
        return sign * value;
...
static TKind lex_next(void)
{
    ...
    if (c == '\'') {
        ...
        if (peek_ch() == '\\') {
            ...
            switch (next_ch()) {
                ...
                default: die("Desteklenmeyen karakter kaçışı"); return TK_ERROR;
            }
        } else {
            value = next_ch();
        }
        ...
        while (peek_ch() != '"' && peek_ch() != -1) {
            ...
            if (peek_ch() == '\\') {
                ...
                switch (next_ch()) {
                    ...
                    case '\'': value = '\''; break;
                    case '0': value = 0; break;
                    default: die("Desteklenmeyen string kaçışı"); return TK_ERROR;
                }
            } else {
                value = next_ch();
            }
    ...
    switch (c) {
        ...
        case '|':
            ...
            return tok=TK_PIPE;
        case '^': return tok=TK_CARET;
        case '~': return tok=TK_TILDE;
        case '<':
            if (peek_ch()=='<') { next_ch(); return tok=TK_SHL; }
            if (peek_ch()=='=') { next_ch(); return tok=TK_LEQ; }
            return tok=TK_LT;
        case '>':
            if (peek_ch()=='>') { next_ch(); return tok=TK_SHR; }
...
static void expect(TKind k) {
    if (tok != k) die("Beklenmeyen token: %d (beklenen %d)", (int)tok, (int)k);
    lex_next();
}
...
static void parse_struct_specifier(int is_union)
{
    ...
    if (!tag[0]) {
        /* parse_struct_body generated the name from the struct count. */
        snprintf(tag, sizeof(tag), "__anon_struct_%d", nstructs - 1);
    }
    s = struct_find(tag);
...
static int64_t parse_const_primary(void)
{
    ...
    if (tok == TK_IDENT && enum_value(tok_ident, &value)) {
        lex_next();
        return value;
    }
    if (eat(TK_LPAREN)) {
...
static int parse_primary(void)
{
    ...
    if (tok == TK_STRING) {
        const char *label = string_add(tok_string);
        lex_next();
        r = alloc_reg();
    ...
    if (tok == TK_IDENT) {
        ...
        if (tok == TK_LPAREN) {
            ...
            { int i;
              for (i = 0; i < nargs && i < MAX_PARAMS; i++)
...
static int parse_postfix(void)
{
    ...
    for (;;) {
        if (tok == TK_LBRACKET) {
            ...
            idx = parse_expr();
            expect(TK_RBRACKET);
            if (base.object_words > 1) {
                emit("    LI   R%d, %d", REG_SCRATCH, base.object_words);
...
static int parse_unary(void)
{
    ...
    if (eat(TK_TILDE)) {
        r = parse_unary();
        r2 = alloc_reg();
        emit("    XOR  R%d, R%d, R%d ; NOT via XOR R0", r2, r, r);
        /* Actually proper NOT: XOR with all-ones. We use: SUB R_r2, R0, R_r (2's complement), then XOR trick */
        emit("    LI   R%d, -1", REG_SCRATCH);
        emit("    XOR  R%d, R%d, R%d", r2, r, REG_SCRATCH);
        free_reg();
        return r2;
    }
    if (eat(TK_STAR)) {
    ...
    if (eat(TK_AMP)) {
        ...
        if (tok != TK_IDENT) die("& sadece değişkenlere uygulanabilir");
        (void)emit_addr_of(tok_ident);
        lex_next();
        return alloc_reg() - 1; /* addr_of already allocated */
    }
...
static int parse_add(void) {
    int r = parse_mul();
    for (;;) {
        if (eat(TK_PLUS))  { int r2=parse_mul(); r=emit_binary_in_left(r,r2,"ADD"); }
        else if (eat(TK_MINUS)) { int r2=parse_mul(); r=emit_binary_in_left(r,r2,"SUB"); }
        else break;
    }
    return r;
}
...
static int parse_shift(void) {
    int r = parse_add();
    for (;;) {
        if (eat(TK_SHL)) { int r2=parse_add(); r=emit_binary_in_left(r,r2,"SHL"); }
        else if (eat(TK_SHR)) { int r2=parse_add(); r=emit_binary_in_left(r,r2,"SHR"); }
        else break;
    }
    return r;
}
...
static int parse_cmp(void) {
    int r = parse_shift();
    for (;;) {
        if      (eat(TK_EQEQ)) { int r2=parse_shift(); r=emit_binary_in_left(r,r2,"CMPEQ"); }
        else if (eat(TK_NEQ))  { int r2=parse_shift(); r=emit_binary_in_left(r,r2,"CMPNE"); }
        else if (eat(TK_LT))   { int r2=parse_shift(); r=emit_binary_in_left(r,r2,"CMPLT"); }
        else if (eat(TK_LEQ))  { int r2=parse_shift(); r=emit_binary_in_left(r,r2,"CMPLE"); }
        else if (eat(TK_GT))   { int r2=parse_shift(); emit("    CMPLT  R%d, R%d, R%d",r,r2,r); free_reg(); }
        else if (eat(TK_GEQ))  { int r2=parse_shift(); emit("    CMPLE  R%d, R%d, R%d",r,r2,r); free_reg(); }
        else break;
    }
    return r;
}
...
/* ── Bit AND/XOR/OR ── */
static int parse_bitand(void) { int r=parse_cmp(); for(;;){if(eat(TK_AMP)){int r2=parse_cmp();r=emit_binary_in_left(r,r2,"AND");}else break;} return r; }
static int parse_bitxor(void) { int r=parse_bitand(); for(;;){if(eat(TK_CARET)){int r2=parse_bitand();r=emit_binary_in_left(r,r2,"XOR");}else break;} return r; }
static int parse_bitor(void)  { int r=parse_bitxor(); for(;;){if(eat(TK_PIPE)){int r2=parse_bitxor();r=emit_binary_in_left(r,r2,"OR");}else break;} return r; }
...
static int parse_cond(void)
{
    ...
    false_lbl = new_label();
    end_lbl = new_label();
    emit("    JZ   R%d, _L%d", cond, false_lbl);
    free_reg();
...
static int parse_expr(void) {
    ...
    if (tok == TK_EQ || tok == TK_PLUSEQ || tok == TK_MINUSEQ ||
        ...
        if (op != TK_EQ) {
            ...
            else                       emit("    DIV  R%d, R%d, R%d", rnew, lhs, rval);
            rval = rnew;
        }
...
static void parse_stmt(int break_lbl, int cont_lbl)
{
    ...
    if (is_decl_start()) {
        ...
        for (;;) {
            ...
            lex_next();
            if (eat(TK_LBRACKET)) {
                array_len = parse_array_bound(1);
                if (array_len == 0)
                    array_len = DEFAULT_ARRAY_WORDS;
                expect(TK_RBRACKET);
            }
                ...
                if (eat(TK_LBRACE)) {
                    ...
                    while (tok != TK_RBRACE && tok != TK_EOF) {
                        ...
                        if (element >= array_len)
                            die("Yerel dizi başlatıcısı boyutu aşıyor");
                        r = parse_expr();
                        emit("    STORE R%d, R%d, %d",
                             r, REG_FP, fp_off + element * object_words);
    ...
    if (eat(TK_RETURN)) {
        ...
        if (func_end_label >= 0) emit("    JMP  _L%d", func_end_label);
        else emit("    RET");
        expect(TK_SEMI);
        return;
    }
    ...
    if (eat(TK_WHILE)) {
        ...
        int r = parse_expr();
        expect(TK_RPAREN);
        emit("    JZ   R%d, _L%d", r, end_lbl);
        free_regs_to(0);
        parse_stmt(end_lbl, loop_lbl);
        emit("    JMP  _L%d", loop_lbl);
        emit_label(end_lbl);
        return;
    }
    if (eat(TK_FOR)) {
        ...
        if (is_decl_start()) {
            int decl_ptr = parse_type_and_ptr();
            char vname[MAX_IDENT]; strncpy(vname, tok_ident, MAX_IDENT-1);
            int array_len = 1;
            int object_words = current_type_words > 0 ? current_type_words : 1;
            int fp_off = 2 + frame_nparams + frame_nlocals;
            lex_next();
            if (eat(TK_LBRACKET)) {
                if (tok != TK_NUM || tok_num <= 0)
                    die("Yerel dizi boyutu sabit pozitif olmalı");
                array_len = (int)tok_num;
                lex_next();
                expect(TK_RBRACKET);
            }
            frame_nlocals += object_words * array_len;
            {
                Sym *local = sym_add_local(vname, fp_off, 0);
                sym_set_object_type(local, decl_ptr);
                local->array_len = array_len;
            }
            if (eat(TK_EQ)) { int r=parse_expr(); emit("    STORE R%d, R%d, %d",r,REG_FP,fp_off); free_regs_to(0); }
            expect(TK_SEMI);
        } else if (tok != TK_SEMI) {
            int r = parse_expr(); free_regs_to(0); (void)r; expect(TK_SEMI);
        } else expect(TK_SEMI);
        /* cond */
        emit_label(cond_lbl);
        if (tok != TK_SEMI) {
        ...
        emit("    JMP  _L%d", body_lbl);
        expect(TK_SEMI);
        /* incr */
        emit_label(incr_lbl);
        if (tok != TK_RPAREN) { int r=parse_expr(); free_regs_to(0); (void)r; }
        emit("    JMP  _L%d", cond_lbl);
        expect(TK_RPAREN);
        emit_label(body_lbl);
        parse_stmt(end_lbl, incr_lbl);
        emit("    JMP  _L%d", incr_lbl);
        emit_label(end_lbl);
        sym_restore(sym_m);
        return;
    }
    if (eat(TK_BREAK)) {
        ...
        emit("    JMP  _L%d", break_lbl);
        expect(TK_SEMI);
        return;
    }
    if (eat(TK_CONTINUE)) {
        if (cont_lbl < 0) die("continue döngü dışında");
        emit("    JMP  _L%d", cont_lbl);
        expect(TK_SEMI);
        return;
    }
    /* İfade deyimi */
...
static void parse_global_decl(const char *name, int decl_ptr)
{
    ...
    if (eat(TK_LBRACKET)) {
        has_array = 1;
        words = parse_array_bound(1);
        if (words == 0)
            words = DEFAULT_ARRAY_WORDS;
        expect(TK_RBRACKET);
    }
    if (current_type_words > 1)
        words *= current_type_words;
    if (tok == TK_EQ && !has_array && !decl_ptr && current_type_is_char) {
    ...
    {
        Sym *s = sym_add_global(name, words);
        s->object_words = current_type_words > 0 ? current_type_words : 1;
        if (current_type_is_struct) {
            strncpy(s->struct_name, current_type, MAX_IDENT - 1);
            s->struct_name[MAX_IDENT - 1] = '\0';
    ...
    if (eat(TK_EQ)) {
        if (eat(TK_LBRACE)) {
            while (tok != TK_RBRACE) {
                if (g->init_count >= g->words)
                    die("Global başlatıcı dizi boyutunu aşıyor");
                if (tok != TK_NUM && tok != TK_CHARCONST)
                    die("Global başlatıcı yalnızca sabit sayılardan oluşabilir");
                g->init[g->init_count++] = tok_num;
                lex_next();
                if (!eat(TK_COMMA))
                    break;
            }
            ...
            const char *label = string_add(tok_string);
            if (decl_ptr) {
                g->init_label[0] = (char *)label;
                g->init_count = 1;
            } else {
                int n = (int)strlen(tok_string);
                if (n + 1 > g->words)
...
static void emit_globals(void)
{
    ...
    for (i = 0; i < nstrings; i++) {
        ...
        for (j = 0; j <= (int)strlen(strings[i].text); j++)
            emit("    .word %d", (unsigned char)strings[i].text[j]);
    }
}
...
int main(int argc, char **argv)
{
    ...
    if (!infile) {
        ...
        fprintf(stderr, "Kullanım: cc kaynak.c -o çıktı.asm\n");
        return 1;
    }
    ...
    printf("cc: %s → %s  (%d etiket üretildi)\n",
           infile, outfile ? outfile : "(stdout)", label_cnt);
    return 0;
}
