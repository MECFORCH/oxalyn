...
module oxalyn_top (
    ...
    // GPU port aralığı: 0xE0–0xFC; UART 0xFD–0xFF'i kullanır.
    // UART, GPU'nin eski 0xE0-0xFF aralığındaki son üç portunu kullanır:
    // GPU 0xE0-0xFC, UART 0xFD=durum/0xFE=TX/0xFF=RX.
    wire        gpu_sel    = (io_addr >= 8'hE0) && (io_addr <= 8'hFC);
    ...
    // Tek portlu BRAM için basit önceliklendirme:
    // GPU DMA isteği geldiğinde GPU adres/veri yolu CPU yolunu kaplar.
    // GPU istemiyorsa CPU'nun normal fetch/load/store yolu kullanılır.
    assign bram_addr  = gpu_ram_req ? gpu_ram_addr[15:0] : mem_addr;
    assign bram_we    = gpu_ram_req ? gpu_ram_wr : mem_we;
    assign bram_wdata = gpu_ram_req ? gpu_ram_wdata : mem_wdata;

    // ─── GPU IRQ → MIP.EXT ────────────────────────────────
    // GPU irq_out → Oxalyn-64 harici kesme girişi
    // GPU IRQ CPU'nun harici kesme girişine ve LED[14]'e gider.
    ...
    assign led[14]   = gpu_irq_latched | gpu_irq_dma; // GPU IRQ göstergesi
