        ...
        if (!rst_n) begin
            ...
            if (req_valid) begin
                if (tag_match) begin
                    ...
                    cache_data[miss_idx]  <= mem_rdata;
                    cache_tag[miss_idx]   <= miss_tag;
...
module oxalyn_cache_ctrl (
    ...
    // D-cache requestleri I-cache miss'lerinden önce servis edilir.  İstek
    // çevrimi boyunca d_req yüksek tutulduğu için grant de kararlı kalır.
    wire        dc_selected = d_req;
    wire        ic_selected = !dc_selected && if_req && ic_stall;
    ...
    oxalyn_icache ic (
        ...
        .mem_addr(ic_mem_addr), .mem_rdata(sram_rdata),
        .mem_ack(sram_ack && ic_selected)
    );
    ...
    always @(*) begin
        if (dc_selected) begin
            sram_addr  = dc_mem_addr;
            sram_we    = dc_mem_we;
            sram_wdata = dc_mem_wdata;
        end else if (ic_selected) begin
