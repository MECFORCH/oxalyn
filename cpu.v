...
module oxalyn_cpu (
    ...
    // Harici kesme hattı (GPU/UART/GPIO gibi çevre birimleri)
...
localparam CAUSE_IRQ_EXT   = 64'h0000_0000_8000_0002;
...
always @(*) begin
    alu_result    = 64'd0;
    branch_taken  = 1'b0;
    branch_target = ex_pc + 16'd1; // varsayılan: sıradaki komut
    ...
    case (ex_op)
        ...
        OP_JMP: begin
            branch_taken  = 1'b1;
            branch_target = ex_pc + ex_imm[15:0];
        end
        OP_JZ: begin
            branch_taken  = (fw_fa == 64'd0);
            branch_target = ex_pc + ex_imm[15:0];
        end
        OP_JNZ: begin
            branch_taken  = (fw_fa != 64'd0);
        ...
        OP_CALL: begin
            branch_taken  = 1'b1;
            branch_target = ex_pc + ex_imm[15:0];
            alu_result    = {48'd0, ex_pc}; // dönüş adresi
        end
        OP_RET: begin
            branch_taken  = 1'b1;
...
always @(*) begin
    ...
    if (ex_valid && ex_op == OP_CALL) begin
        ...
        mem_we    = 1'b1;
        mem_wdata = {48'd0, ex_pc}; // dönüş adresi
    end
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // Store verisi: STORE için fw_fa (fd alanı = RS1 = veri kaynağı)
        // OUT için de fw_fa (fd alanı = RS1)
        mem_store_data <= (ex_op == OP_STORE || ex_op == OP_OUT) ? ex_rv_fa : fw_fb;
        ...
        if (flush_id_ex || !id_valid) begin
            ...
            ex_rv_fb          <= id_rfb;
            ex_imm            <= id_imm;
            ex_pc             <= id_pc + 16'd1; // fetch sonrası PC
        ...
        // ────────────────────────────────────────────
        if (branch_taken && ex_valid && !ex_pred_correct_w) begin
            // Yanlış tahmin veya tahmin edilmemiş dal → gerçek hedefe git
            pc_if <= branch_target;
...
wire predict_redirect = id_is_backward && !stall;
...
always @(*) begin
    ...
    flush_if_id = (branch_taken && ex_valid && !ex_pred_correct_w) ? 1'b1 : 1'b0;
    flush_id_ex = ((branch_taken && ex_valid && !ex_pred_correct_w) || load_use_hazard) ? 1'b1 : 1'b0;
end
...
module oxalyn_cpu_tb;
    ...
    initial begin
        $dumpfile("oxalyn_pipeline.vcd");
        $dumpvars(0, oxalyn_cpu_tb);
    end
endmodule
